...
namespace Taxer.DBA
{
public class AgreementTypeDBAccess
{
	...
	public List<Heir> GetListHeir(int agreementTypeID)
	{
		...
		SqlParameter[] parametersAgreementType = new SqlParameter[]
		{
			new SqlParameter("@AgreementTypeID", agreementTypeID)
		};

		//Lets get the list of Heir records from database
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Heir_GetList_UseInAgreementType", CommandType.StoredProcedure, parametersAgreementType))
		{
	...
	public Int64 Insert(AgreementType agreementType)
	{
		SqlParameter[] parametersAgreementType = new SqlParameter[]
		{
			new SqlParameter("@Title", (agreementType.Title != null) ? agreementType.Title : (object)DBNull.Value)
		};
		agreementType.AgreementTypeID = Convert.ToInt32(SqlDBHelper.ExecuteScalar("AgreementType_Insert", CommandType.StoredProcedure, parametersAgreementType));
		return agreementType.AgreementTypeID;
	}
	...
	public bool Update(AgreementType agreementType)
	{
		SqlParameter[] parametersAgreementType = new SqlParameter[]
		{
			new SqlParameter("@AgreementTypeID", agreementType.AgreementTypeID),
			new SqlParameter("@Title", (agreementType.Title != null) ? agreementType.Title : (object)DBNull.Value)
		};
		return SqlDBHelper.ExecuteNonQuery("AgreementType_Update", CommandType.StoredProcedure, parametersAgreementType);
	}
	...
	public bool Delete(int agreementTypeID)
	{
		...
		return SqlDBHelper.ExecuteNonQuery("AgreementType_Delete", CommandType.StoredProcedure, parametersAgreementType);
	}
	...
	public AgreementType GetDetails(int agreementTypeID)
	{
		...
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("AgreementType_GetDetails", CommandType.StoredProcedure, parametersAgreementType))
		{
			if (table.Rows.Count == 1)
			{
				DataRow row = table.Rows[0];

				agreementType.AgreementTypeID = (row["AgreementTypeID"] != DBNull.Value) ? Convert.ToInt32(row["AgreementTypeID"]) : 0 ;
				agreementType.Title = row["Title"].ToString();
			}
		}
	...
	{
		List<AgreementType> listAgreementType = null;

		using (DataTable table = SqlDBHelper.ExecuteSelectCommand("AgreementType_GetListAll", CommandType.StoredProcedure))
		{
			if (table.Rows.Count > 0)
			{
				listAgreementType = new List<AgreementType>();
	...
	public bool Exists(int agreementTypeID)
	{
		...
		return (SqlDBHelper.ExecuteScalar("AgreementType_Exists", CommandType.StoredProcedure, parametersAgreementType)>0);
	}
	...
	{
		List<AgreementType> listAgreementType = new List<AgreementType>();


		SqlParameter[] parametersAgreementType = new SqlParameter[]
		{
			new SqlParameter("@AgreementTypeID", agreementType.AgreementTypeID),
			new SqlParameter("@Title", agreementType.Title),
		};
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("AgreementType_SearchLike", CommandType.StoredProcedure, parametersAgreementType))
		{
			if (table.Rows.Count > 0)
			{
				foreach (DataRow row in table.Rows)
				{
					AgreementType tmpAgreementType = new AgreementType();
					tmpAgreementType.AgreementTypeID = (row["AgreementTypeID"] != DBNull.Value) ? Convert.ToInt32(row["AgreementTypeID"]) : 0 ;
					tmpAgreementType.Title = row["Title"].ToString();

					listAgreementType.Add(tmpAgreementType);
				}
			}
		}
