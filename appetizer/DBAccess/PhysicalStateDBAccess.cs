...
namespace Taxer.DBA
{
public class PhysicalStateDBAccess
{
	...
	public List<Heir> GetListHeir(int physicalStateID)
	{
		...
		SqlParameter[] parametersPhysicalState = new SqlParameter[]
		{
			new SqlParameter("@PhysicalStateID", physicalStateID)
		};

		//Lets get the list of Heir records from database
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Heir_GetList_UseInPhysicalState", CommandType.StoredProcedure, parametersPhysicalState))
		{
	...
	public Int64 Insert(PhysicalState physicalState)
	{
		SqlParameter[] parametersPhysicalState = new SqlParameter[]
		{
			new SqlParameter("@Title", physicalState.Title)
		};
		physicalState.PhysicalStateID = Convert.ToInt32(SqlDBHelper.ExecuteScalar("PhysicalState_Insert", CommandType.StoredProcedure, parametersPhysicalState));
		return physicalState.PhysicalStateID;
	}
	...
	public bool Update(PhysicalState physicalState)
	{
		SqlParameter[] parametersPhysicalState = new SqlParameter[]
		{
			new SqlParameter("@PhysicalStateID", physicalState.PhysicalStateID),
			new SqlParameter("@Title", physicalState.Title)
		};
		return SqlDBHelper.ExecuteNonQuery("PhysicalState_Update", CommandType.StoredProcedure, parametersPhysicalState);
	}
	...
	public bool Delete(int physicalStateID)
	{
		...
		return SqlDBHelper.ExecuteNonQuery("PhysicalState_Delete", CommandType.StoredProcedure, parametersPhysicalState);
	}
	...
	public PhysicalState GetDetails(int physicalStateID)
	{
		...
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("PhysicalState_GetDetails", CommandType.StoredProcedure, parametersPhysicalState))
		{
			if (table.Rows.Count == 1)
			{
				DataRow row = table.Rows[0];

				physicalState.PhysicalStateID = (row["PhysicalStateID"] != DBNull.Value) ? Convert.ToInt32(row["PhysicalStateID"]) : 0 ;
				physicalState.Title = row["Title"].ToString();
			}
		}
	...
	{
		List<PhysicalState> listPhysicalState = null;

		using (DataTable table = SqlDBHelper.ExecuteSelectCommand("PhysicalState_GetListAll", CommandType.StoredProcedure))
		{
			if (table.Rows.Count > 0)
			{
				listPhysicalState = new List<PhysicalState>();
				...
				foreach (DataRow row in table.Rows)
				{
					...
					physicalState.PhysicalStateID = (row["PhysicalStateID"] != DBNull.Value) ? Convert.ToInt32(row["PhysicalStateID"]) : 0 ;
					physicalState.Title = row["Title"].ToString();
					listPhysicalState.Add(physicalState);
				}
			}
		}

		return listPhysicalState;
	}
	...
	public bool Exists(int physicalStateID)
	{
		...
		return (SqlDBHelper.ExecuteScalar("PhysicalState_Exists", CommandType.StoredProcedure, parametersPhysicalState)>0);
	}
	...
	{
		List<PhysicalState> listPhysicalState = new List<PhysicalState>();
		...
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("PhysicalState_SearchLike", CommandType.StoredProcedure, parametersPhysicalState))
		{
			if (table.Rows.Count > 0)
			{
				foreach (DataRow row in table.Rows)
				{
					PhysicalState tmpPhysicalState = new PhysicalState();
					tmpPhysicalState.PhysicalStateID = (row["PhysicalStateID"] != DBNull.Value) ? Convert.ToInt32(row["PhysicalStateID"]) : 0 ;
					tmpPhysicalState.Title = row["Title"].ToString();

					listPhysicalState.Add(tmpPhysicalState);
				}
			}
		}
