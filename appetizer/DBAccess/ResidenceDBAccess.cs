...
namespace Taxer.DBA
{
public class ResidenceDBAccess
{
	...
	public List<Dead> GetListDead(int residenceID)
	{
		...
		SqlParameter[] parametersResidence = new SqlParameter[]
		{
			new SqlParameter("@ResidenceID", residenceID)
		};

		//Lets get the list of Dead records from database
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Dead_GetList_UseInResidence", CommandType.StoredProcedure, parametersResidence))
		{
	...
	public List<Heir> GetListHeir(int residenceID)
	{
		...
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Heir_GetList_UseInResidence", CommandType.StoredProcedure, parametersResidence))
		{
			//check if any record exists or not
			if (table.Rows.Count > 0)
			{
	...
	public Int64 Insert(Residence residence)
	{
		SqlParameter[] parametersResidence = new SqlParameter[]
		{
			new SqlParameter("@Title", residence.Title)
		};
		residence.ResidenceID = Convert.ToInt32(SqlDBHelper.ExecuteScalar("Residence_Insert", CommandType.StoredProcedure, parametersResidence));
	...
	public bool Update(Residence residence)
	{
		...
		return SqlDBHelper.ExecuteNonQuery("Residence_Update", CommandType.StoredProcedure, parametersResidence);
	}
	...
	public bool Delete(int residenceID)
	{
		...
		return SqlDBHelper.ExecuteNonQuery("Residence_Delete", CommandType.StoredProcedure, parametersResidence);
	}
	...
	public Residence GetDetails(int residenceID)
	{
		...
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Residence_GetDetails", CommandType.StoredProcedure, parametersResidence))
		{
			if (table.Rows.Count == 1)
			{
				DataRow row = table.Rows[0];

				residence.ResidenceID = (row["ResidenceID"] != DBNull.Value) ? Convert.ToInt32(row["ResidenceID"]) : 0 ;
				residence.Title = row["Title"].ToString();
			}
		}
	...
	{
		List<Residence> listResidence = null;

		using (DataTable table = SqlDBHelper.ExecuteSelectCommand("Residence_GetListAll", CommandType.StoredProcedure))
		{
			if (table.Rows.Count > 0)
			{
				listResidence = new List<Residence>();
	...
	public bool Exists(int residenceID)
	{
		...
		return (SqlDBHelper.ExecuteScalar("Residence_Exists", CommandType.StoredProcedure, parametersResidence)>0);
	}
	...
	{
		List<Residence> listResidence = new List<Residence>();
		...
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Residence_SearchLike", CommandType.StoredProcedure, parametersResidence))
		{
			if (table.Rows.Count > 0)
			{
				foreach (DataRow row in table.Rows)
				{
					Residence tmpResidence = new Residence();
					tmpResidence.ResidenceID = (row["ResidenceID"] != DBNull.Value) ? Convert.ToInt32(row["ResidenceID"]) : 0 ;
					tmpResidence.Title = row["Title"].ToString();

					listResidence.Add(tmpResidence);
				}
			}
		}
