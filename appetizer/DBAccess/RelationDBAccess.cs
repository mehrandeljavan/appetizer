...
public class RelationDBAccess
{
	...
	public List<Heir> GetListHeir(int relationID)
	{
		...
		SqlParameter[] parametersRelation = new SqlParameter[]
		{
			new SqlParameter("@RelationID", relationID)
		};

		//Lets get the list of Heir records from database
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Heir_GetList_UseInRelation", CommandType.StoredProcedure, parametersRelation))
		{
	...
	public Int64 Insert(Relation relation)
	{
		SqlParameter[] parametersRelation = new SqlParameter[]
		{
			new SqlParameter("@RelationID", relation.RelationID),
			new SqlParameter("@RelationLevel", (relation.RelationLevel != null) ? relation.RelationLevel : (object)DBNull.Value),
			new SqlParameter("@Degree", (relation.Degree != null) ? relation.Degree : (object)DBNull.Value),
			new SqlParameter("@ParentRelationID", (relation.ParentRelationID != null) ? relation.ParentRelationID : (object)DBNull.Value),
			new SqlParameter("@Title", relation.Title)
		};
		return SqlDBHelper.ExecuteScalar("Relation_Insert", CommandType.StoredProcedure, parametersRelation);
	}
	...
	public bool Update(Relation relation)
	{
		...
		return SqlDBHelper.ExecuteNonQuery("Relation_Update", CommandType.StoredProcedure, parametersRelation);
	}
	...
	public bool Delete(int relationID)
	{
		...
		return SqlDBHelper.ExecuteNonQuery("Relation_Delete", CommandType.StoredProcedure, parametersRelation);
	}
	...
	public Relation GetDetails(int relationID)
	{
		...
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Relation_GetDetails", CommandType.StoredProcedure, parametersRelation))
		{
			if (table.Rows.Count == 1)
			{
				DataRow row = table.Rows[0];

				relation.RelationID = (row["RelationID"] != DBNull.Value) ? Convert.ToInt32(row["RelationID"]) : 0 ;
				relation.RelationLevel = (row["RelationLevel"] != DBNull.Value) ? Convert.ToInt32(row["RelationLevel"]) : 0 ;
				relation.Degree = (row["Degree"] != DBNull.Value) ? Convert.ToInt32(row["Degree"]) : 0 ;
				relation.ParentRelationID = (row["ParentRelationID"] != DBNull.Value) ? Convert.ToInt32(row["ParentRelationID"]) : 0 ;
				relation.Title = row["Title"].ToString();
			}
		}
	...
	public List<Relation> GetListAll()
	{
		...
		using (DataTable table = SqlDBHelper.ExecuteSelectCommand("Relation_GetListAll", CommandType.StoredProcedure))
		{
			if (table.Rows.Count > 0)
			{
				listRelation = new List<Relation>();
	...
	public bool Exists(int relationID)
	{
		...
		return (SqlDBHelper.ExecuteScalar("Relation_Exists", CommandType.StoredProcedure, parametersRelation)>0);
	}
	...
	public List<Relation> SearchLike(Relation relation)
	{
		...
		{
			new SqlParameter("@RelationID", relation.RelationID),
			new SqlParameter("@RelationLevel", relation.RelationLevel),
			new SqlParameter("@Degree", relation.Degree),
			new SqlParameter("@ParentRelationID", relation.ParentRelationID),
			new SqlParameter("@Title", relation.Title),
		};
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Relation_SearchLike", CommandType.StoredProcedure, parametersRelation))
		{
			if (table.Rows.Count > 0)
			{
				foreach (DataRow row in table.Rows)
				{
					...
					tmpRelation.RelationID = (row["RelationID"] != DBNull.Value) ? Convert.ToInt32(row["RelationID"]) : 0 ;
					tmpRelation.RelationLevel = (row["RelationLevel"] != DBNull.Value) ? Convert.ToInt32(row["RelationLevel"]) : 0 ;
					tmpRelation.Degree = (row["Degree"] != DBNull.Value) ? Convert.ToInt32(row["Degree"]) : 0 ;
					tmpRelation.ParentRelationID = (row["ParentRelationID"] != DBNull.Value) ? Convert.ToInt32(row["ParentRelationID"]) : 0 ;
					tmpRelation.Title = row["Title"].ToString();

					listRelation.Add(tmpRelation);
				}
			}
		}
